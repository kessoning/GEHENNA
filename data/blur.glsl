...
void main() {
    ...
    vec3  c = texture2D(imageInput, uv).rgb;
    ...
    float  Z = 0.0;
    float sigma = 0.03 + texture2D(depthMask, uv).r * 7.0;
    ...
    for (int  i = -kSize; i <= kSize; ++i) {
        for(int  j = -kSize; j <= kSize; ++j) {
            final_colour += kernel[kSize + j] * kernel[kSize + i] * texture2D(imageInput, (gl_FragCoord.xy + vec2(float(i), float(j))) / iResolution.xy).rgb;
        }
    }
